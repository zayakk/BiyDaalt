...
from appbackend.models import Product
...
# }
    ...
    try:
        ...
        # Format the response
        action = "get_products"
        respdata = list(products)  # List of products to send back in response
        resp = sendResponse(request, 200, respdata, action)  # Sending response with code 200 (OK)
        
    except Exception as e:
        # If an exception occurs, handle it gracefully
        action = "get_products"
        respdata = [{"error": str(e)}]
        resp = sendResponse(request, 5000, respdata, action)  # Internal server error
...
# }
    ...
    if request.method == 'PUT':
        try:
            ...
            # Update the product fields based on request data
            ...
            respdata = [{"message": "Product updated successfully."}]
            resp = sendResponse(request, 200, respdata, action)
            ...
            resp = sendResponse(request, 404, respdata, action)
        ...
        action = "update_product"
        respdata = [{"error": "Invalid request method."}]
        resp = sendResponse(request, 405, respdata, action)
...
# }
    ...
    if request.method == 'DELETE':
        try:
            ...
            # Prepare success response
            action = "delete_product"
            respdata = [{"message": "Product deleted successfully."}]
            resp = sendResponse(request, 200, respdata, action)
